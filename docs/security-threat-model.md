...
| Proxy → durable-ingest → RabbitMQ                      | Browser batch payload                                                               | Server-side pre-persistence sanitizer; no inbound browser headers persisted; publisher confirm required before `202`                              | Stage 20.5                                                                                             |
| Delivery-worker → OpenObserve                          | Sanitized durable batch                                                             | Server-side OpenObserve ingest token secret; manual ACK only after OpenObserve success; retry/DLQ; at-least-once only                             | Stage 20.5                                                                                             |
| OpenObserve ingestion pipeline                         | Payload delivered by worker                                                         | VRL sanitization backstop (defense-in-depth, not the primary control)                                                                             | `infrastructure/openobserve/sanitization/*.vrl`, Stage 9                                               |
