...
test.describe("browser app fixture smoke", () => {
  ...
  test("runtime-config lifecycle controls are visible", async ({ page }) => {
    await page.goto("/");
    for (const id of [
      "initialize",
  ...
  test("the real OpenObserve adapter initializes and keeps the fixture healthy", async ({
    page,
  }) => {
    // OpenObserve integration: a real adapter is wired by default now, so this fixture
    // (schema-valid, enabled, but pointing at a non-resolving fake site)
    // reaches "active"; real ingestion requests to that fake site fail
    // silently without surfacing as a page error.
    const pageErrors = [];
    page.on("pageerror", (error) => pageErrors.push(error));
  ...
  test("HTTP test service health endpoint responds", async ({ request }) => {
