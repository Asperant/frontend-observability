...
// OpenObserve integration real-adapter behavior, run across all three engines
...
// tests/e2e/openobserve-ingestion.spec.js (chromium + firefox: see
// playwright-browser-adapter.config.js for why webkit is excluded there); this file
...
test.describe("OpenObserve integration real adapter behavior (all engines)", () => {
  ...
  test("shutdown stops telemetry and a real reinitialize resumes the adapter", async ({ page }) => {
    ...
    // fixture for an unrelated runtime-config test. Genuine post-reinit ingestion
    // traffic is confirmed by tests/e2e/openobserve-ingestion.spec.js against a real,
