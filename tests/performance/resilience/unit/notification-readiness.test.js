...
describe("buildProbeId", () => {
  it("embeds a random uuid by default", () => {
    ...
    expect(id).toMatch(/^resilience-notification-probe-[0-9a-f-]{36}$/);
  });

  it("accepts an injected uuid for deterministic tests", () => {
    expect(buildProbeId("fixed-uuid")).toBe("resilience-notification-probe-fixed-uuid");
  });
...
describe("parseProbeEvents", () => {
  const probeId = "resilience-notification-probe-abc";
