...
// scripts/security/verify-security-acceptance.mjs owns that).
...
import { MAX_CONTROL_BODY_BYTES } from "../../../packages/browser-observability/src/runtime-control/constants.js";
import { hasDuplicateObjectKeys } from "../../../packages/browser-observability/src/runtime-control/duplicate-keys.js";
import {
...
const RUN_ID = `security-acceptance-${Date.now().toString(36)}`;
...
function isRejectedByBrowserControlValidation(rawText, now = new Date()) {
  if (new TextEncoder().encode(rawText).byteLength > MAX_CONTROL_BODY_BYTES) return true;
  if (hasDuplicateObjectKeys(rawText)) return true;
  ...
  const lifetime = validateControlLifetime(parsed, now);
...
export async function checkRuntimeControlAdversarialDocuments() {
  ...
  try {
    for (const [label, rawText] of ADVERSARIAL_CONTROL_DOCS) {
      ...
      const rejected = isRejectedByBrowserControlValidation(rawText);
      evidence.push(
        `${label}: public status=${served.statusCode} rejectedByBrowserValidation=${rejected}`,
      );
      if (!rejected) {
        findings.push(
...
export async function checkKillSwitchClosesUpstream() {
  const findings = [];
  const canary = `security-acceptance-killswitch-canary-${RUN_ID}`;
...
export function checkDockerCrossContainerIsolation() {
  ...
  // browser-app's runtime image is nginx-unprivileged (no node binary);
  // http-test-service's is the node-based HTTP test service (see their respective
  // healthchecks in infrastructure/docker/compose.yaml for the same tool
  // choice per image).
  const demoFrontendProbe = execWgetProbe("browser-app");
  evidence.push(`browser-app -> openobserve admin API: ${demoFrontendProbe}`);
  if (demoFrontendProbe.startsWith("REACHABLE:")) {
    findings.push(
      "browser-app container can reach OpenObserve's admin API directly (network isolation gap)",
    );
  }

  const httpTestServiceProbe = execProbe("http-test-service", NODE_REACHABILITY_PROBE_SCRIPT);
  evidence.push(
    `http-test-service -> openobserve admin API: ${httpTestServiceProbe.stdout || httpTestServiceProbe.stderr}`,
  );
  if (httpTestServiceProbe.stdout.startsWith("REACHABLE:")) {
    findings.push(
      "http-test-service container can reach OpenObserve's admin API directly (network isolation gap)",
    );
  }
