...
export function validateAdmissionRequest({ method, url, headers, rawBody, maxBytes }) {
  const parsedUrl = new URL(url, "http://durable-ingest.internal");
...
export function buildDurableMessage({ signal, rawBody, receivedAt = new Date() }) {
  const decoded = safeDecode(rawBody);
  const sanitizedPayload = sanitizeBrowserBatch(decoded, { signal });
