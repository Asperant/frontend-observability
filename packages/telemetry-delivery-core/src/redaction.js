...
function enforceSignalShape(event, signal) {
  if (signal === "rum") {
    normalizeRumIdentityFields(event);
    ...
    if (typeof event.session_id !== "string" || event.session_id.length === 0) {
      throw new SanitizationError("missing_session_id");
    }
  }
  if (signal === "logs") {
...
function normalizeRumIdentityFields(event) {
  if (typeof event.session_id !== "string" && typeof event.session?.id === "string") {
    event.session_id = event.session.id;
  }
  if (typeof event.view_id !== "string" && typeof event.view?.id === "string") {
    event.view_id = event.view.id;
  }
  if (typeof event.action_id !== "string" && typeof event.action?.id === "string") {
    event.action_id = event.action.id;
  }
  if (typeof event.error_id !== "string" && typeof event.error?.id === "string") {
    event.error_id = event.error.id;
  }
}
...
function sanitizeString(input) {
  ...
  value = value.replace(LONG_NUMERIC_ID_PATTERN, "[REDACTED_ID]");
