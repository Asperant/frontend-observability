// recovery backup/restore/upgrade/rollback/logical-restore proof for the
// local lab. Runtime archives, compose files, logs, and raw exports are
// written only under .runtime/recovery/ and are never committed. The
...
async function ingest(baseUrl, auth, stream, marker) {
  ...
  const record = {
    ...
    session_id: `recovery-session-${marker}`,
    view_id: `recovery-view-${marker}`,
    marker,
    type: "view",
    message: `recovery marker ${marker}`,
    level: "info",
    error_type: "None",
    error_source_type: "recovery",
    view_loading_time: 1,
    ...
    resource_url: "https://localhost:8443/recovery",
    action_target_name: "recovery",
  };
  await assertOk(
...
function runRecoveryStreamGovernance() {
...
// pass silently (the recovery audit finding against the prior version).
...
async function createDashboardFixture(baseUrl, auth) {
  ...
  const body = buildStarterDashboardBody(
    { ...starter, title: `Recovery ${starter.title}` },
    queries,
...
async function createAlertFixture(baseUrl, auth) {
  ...
  await apiFetch(baseUrl, auth, `/api/${ORG_ID}/alerts/destinations?module=alert`, {
    ...
    body: JSON.stringify({
      ...
      metadata: { lab_only: "true", recovery: "true" },
    }),
  });
  ...
  await assertOk(
    await apiFetch(baseUrl, auth, `/api/v2/${ORG_ID}/alerts?folder=default`, {
      method: "POST",
      body: JSON.stringify({ ...alert, name: `recovery-${alert.name}` }),
    }),
      ...
      body: JSON.stringify([
        {
          ...
          error_message: "recovery redaction alice@example.invalid",
      ...
      body: JSON.stringify([
        {
          ...
          env: DEMO_IDENTITY.environment,
          marker: dropMarker,
          message: `recovery ${"api" + "_key"}="not-a-real-secret-canary-value-000"`,
        },
      ]),
    }),
    "ingest drop smoke",
...
async function bootstrapCanonicalStreams(baseUrl, auth) {
  for (const stream of ["_rumdata", "_rumlog"]) {
    await apiFetch(baseUrl, auth, `/api/${ORG_ID}/${stream}/_json`, {
      method: "POST",
      body: JSON.stringify([
        { _timestamp: Date.now() * 1000, _bootstrap: "recovery-lazy-stream-create" },
      ]),
    });
  }
...
async function createFixtureEnvironment(baseUrl, auth, marker) {
  ...
  if (!readBack.pass) {
    throw new Error(`recovery source settings read-back failed: ${readBack.findings.join("; ")}`);
  }
  const sanitization = await provisionSanitization({ baseUrl, auth });
  if (!sanitization.pass) {
    throw new Error(`recovery sanitization provision failed: ${sanitization.findings.join("; ")}`);
  }
  ...
  if (!rumdataVisible || !rumlogVisible) throw new Error("recovery fixture markers not visible");
}
...
async function runRecoveryProof() {
  assertExactLabToolchain("test:recovery");
  const runId = new Date().toISOString().replace(/[:.]/g, "-");
  const runDir = join(repoRoot, ".runtime/recovery", runId);
  ...
  result.sessionReplay = {
    replayRegressionClosed: true,
    sessionReplayStreamAbsent: !(await listStreamNames(mainBaseUrl, auth)).includes(
      "_sessionreplay",
    ),
  };
  result.streamGovernance = runRecoveryStreamGovernance();
  ...
  const resultPath = join(runDir, "recovery-results.json");
  writeFileSync(resultPath, `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 });
  const committedPath = join(
    repoRoot,
    "infrastructure/performance/openobserve-recovery-results.json",
  );
  writeFileSync(
    committedPath,
...
if (isMainModule) {
  try {
    const { resultPath, committedPath, result } = await runRecoveryProof();
    console.log(`recovery proof ${result.decision}`);
    console.log(`  runtime: ${resultPath}`);
    console.log(`  committed summary: ${committedPath}`);
  } catch (error) {
    console.error(`recovery proof FAILED: ${error.message}`);
    process.exit(1);
  }
}
