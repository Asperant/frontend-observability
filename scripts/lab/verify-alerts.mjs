...
function runCoverage() {
  const result = spawnSync(
    ...
    [
      ...
      "lab",
      "tests/lab/alerts",
      "tests/fixtures/apps/http-test-service/tests/server.test.js",
      "--coverage",
...
async function runBrowserCanary(browserType) {
  ...
  const context = await browser.newContext({
    ...
    extraHTTPHeaders: { "x-alert-governance-alert-canary": browserType.name() },
  });
...
function companyAlertBody(owner, queryManifest) {
  ...
  return {
    ...
    name: `alert-governance-company-alert-${Date.now()}`,
...
export async function verifyAlerts() {
  assertExactLabToolchain("test:alerts");
  log("test:alerts");
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  try {
    await verifyAlerts();
    log("test:alerts PASSED");
  } catch (error) {
    logError(`test:alerts FAILED: ${error.message}`);
    process.exit(1);
  }
}
