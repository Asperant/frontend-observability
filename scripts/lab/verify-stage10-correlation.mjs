...
export async function verifyStage10Correlation() {
  const { email, password } = readAdminCredentials();
  const auth = basicAuthHeader(email, password);
  log("verify:stage10:correlation — driving Chromium and Firefox through correlated telemetry...");
  const findings = [
...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:stage10:correlation");
    const result = await verifyStage10Correlation();
    if (result.pass) {
      log("test:stage10:correlation PASSED.");
    } else {
      logError("test:stage10:correlation FAILED:");
      for (const finding of result.findings) logError(`  - ${finding}`);
    }
    process.exit(result.pass ? 0 : 1);
  } catch (error) {
    logError(`test:stage10:correlation FAILED: ${error.message}`);
    process.exit(1);
  }
}
