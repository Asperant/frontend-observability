...
export async function verifyStage14RuntimeControl() {
  const findings = [];

  log("stage14: normal control refresh flow (chromium + firefox)...");
  await checkNormalFlow(findings);

  log("stage14: browser kill switch...");
  await checkBrowserKillSwitch(findings);

  log("stage14: proxy kill switch...");
  await checkProxyKillSwitch(findings);

  log("stage14: operator command (full on/off sequence)...");
  await checkOperatorCommand(findings);

  log("stage14: expiry fail-closed...");
  await checkExpiry(findings);

  log("stage14: invalid/rollback control documents...");
  await checkInvalidDocuments(findings);

  log("stage14: lifecycle (duplicate init, consent, shutdown/resume)...");
...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:stage14:runtime-control");
    const result = await verifyStage14RuntimeControl();
    if (result.pass) {
      log("\nStage 14 runtime-control verification PASSED.");
    } else {
      logError("\nStage 14 runtime-control verification FAILED:");
      for (const finding of result.findings) logError(`  - ${finding}`);
    }
    process.exit(result.pass ? 0 : 1);
  } catch (error) {
    logError(`test:stage14:runtime-control FAILED: ${error.message}`);
    process.exit(1);
  }
}
