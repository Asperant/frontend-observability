...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:stage9:leakage");
    const result = await verifySanitizationLeakage();
    if (result.pass) {
      log(
        `test:stage9:leakage PASSED. response_counts=${JSON.stringify(result.responseCounts)} pipeline_failure_counts=${JSON.stringify(result.pipelineFailureCounts)}`,
      );
    } else {
      logError("test:stage9:leakage FAILED:");
      for (const finding of result.findings) logError(`  - ${finding}`);
    }
    process.exit(result.pass ? 0 : 1);
  } catch (error) {
    logError(`test:stage9:leakage FAILED: ${error.message}`);
    process.exit(1);
  }
}
