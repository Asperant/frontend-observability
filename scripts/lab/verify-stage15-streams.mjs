// `pnpm test:stage15:streams` — Stage 15 acceptance gate. Static test
...
//      schema contracts, with replay-absence and Stage 10 correlation
//      checks.
...
function assertNoSecretLeak(findings, secretValues) {
  ...
  for (const secret of secretValues) {
    if (secret && haystack.includes(secret)) {
      throw new Error("INTERNAL: a Stage 15 finding string contained a raw admin credential.");
    }
  }
}
...
async function verifyDisposableLifecycle(auth, testRunId) {
  ...
  const ingest = await ingestJson(auth, streamName, [
    { message: "stage15-disposable-canary", level: "info", run: testRunId },
  ]);
  if (!ingest.ok) {
    findings.push(
...
// the real, already-shipped Stage 9 client-side sanitizer
...
// "stage15-chromium-<8 chars>-<6 chars>" id (exactly 32 characters end to
// end) still vanished (BASE64_TOKEN_PATTERN) — while the shorter
// "stage15-firefox-..." variant (31 characters) survived untouched. A
...
async function runBrowserCanary(browserLabel, browserType, auth) {
  ...
  // Stage 10 correlation must still be attached to real actions.
  for (const hit of actions.hits) {
    if (!hit.chicek_correlation_session_id || !hit.chicek_correlation_epoch_id) {
      findings.push(`[${browserLabel}] real action record missing Stage 10 correlation fields.`);
    }
  }
...
export async function verifyStage15Streams() {
  const findings = [];
  const secretValues = readAdminSecretValues();
  const auth = readAdminAuthHeader();

  log("test:stage15:streams — running coverage-checked unit suite for scripts/lab/streams/...");
  ...
  // (the roadmap's own gate order runs `test:stage15:streams` *before*
  ...
  log("test:stage15:streams — provisioning idempotency (two runs)...");
  ...
  log("test:stage15:streams — canonical stream settings/schema/pipeline read-back...");
  ...
  log("test:stage15:streams — canonical destructive guard...");
  findings.push(...(await verifyCanonicalDestructiveGuardBlocked(auth)));

  log("test:stage15:streams — disposable stream lifecycle...");
  ...
  log("test:stage15:streams — management-plane isolation...");
  findings.push(...(await verifyManagementIsolation(auth)));

  log("test:stage15:streams — Chromium real schema canary...");
  findings.push(...(await runBrowserCanary("chromium", chromium, auth)));

  log("test:stage15:streams — Firefox real schema canary...");
...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:stage15:streams");
    const result = await verifyStage15Streams();
    if (result.pass) {
      log("\n✔ Stage 15 stream lifecycle verification PASSED.");
    } else {
      logError("\n✖ Stage 15 stream lifecycle verification FAILED:");
      for (const finding of result.findings) logError(`  - ${finding}`);
    }
    process.exit(result.pass ? 0 : 1);
  } catch (error) {
    logError(`test:stage15:streams FAILED: ${error.message}`);
    process.exit(1);
  }
}
