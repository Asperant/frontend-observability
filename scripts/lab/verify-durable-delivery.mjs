...
import { holdDelivery, purgeQueues, resumeDelivery } from "./delivery-ops.mjs";
...
function rumEvent(marker) {
  ...
  return {
    ...
    view: { id: viewId, url: `https://localhost:8443/durable_delivery?secret=drop#x` },
  };
}
...
function logEvent(marker) {
  return {
    date: Date.now(),
    marker,
    message: "durable_delivery durable log",
    status: "info",
    service: "browser-app",
    env: "lab",
    version: "2026.07.1",
...
function writeEvidence(name, value) {
  mkdirSync(DURABLE_DELIVERY_RUNTIME_DIR, { recursive: true });
  const path = join(DURABLE_DELIVERY_RUNTIME_DIR, name);
...
function acceptedRumEvent(marker, canaries = {}) {
  return {
    ...
    view: {
      ...
      url: `https://localhost:8443/durable_delivery_volume?query=${canaries.query ?? "drop"}#${canaries.fragment ?? "drop"}`,
    },
  };
}
...
async function optionalOpenObserveRecentHaystack(stream, limit = 500) {
  try {
    return await openObserveRecentHaystack(stream, limit);
  } catch (error) {
    if (String(error?.message ?? "").includes("Search stream not found")) return "";
    throw error;
  }
}
...
async function verifyDurableDelivery() {
  const findings = [];
  const accounting = {
    durablyAccepted: 0,
    delivered: 0,
  ...
  const directOpenObserve = await requestHttps(RUM_PATH, {
    ...
    body: JSON.stringify(rumEvent("durable_delivery-direct-path-check")),
  });
  if (directOpenObserve.statusCode !== 202) {
    findings.push(`telemetry admission returned ${directOpenObserve.statusCode}, expected 202.`);
  } else {
  ...
  const badPayload = await requestHttps(RUM_PATH, {
    ...
    headers: ingestionHeaders(),
    body: JSON.stringify({
      ...rumEvent("durable_delivery-secret"),
  ...
  const marker = `durable_delivery-${crypto.randomUUID()}`;
  ...
  await waitForHealthy({
    services: ["rabbitmq", "telemetry-ingest", "reverse-proxy"],
    timeoutMs: 60_000,
  });
  const outageMarker = `durable_delivery-outage-${crypto.randomUUID()}`;
  ...
  const rabbitDown = (() => {
    ...
    return requestHttps(LOGS_PATH, {
      method: "POST",
      headers: ingestionHeaders(),
      body: JSON.stringify(logEvent(`durable_delivery-rabbit-down-${crypto.randomUUID()}`)),
    });
  })();
  ...
  await waitForHealthy({
    services: ["rabbitmq", "telemetry-ingest", "telemetry-delivery-worker"],
    timeoutMs: 120_000,
  });
...
async function verifyQueueStoragePrivacy() {
  const findings = [];
  purgeQueues({ confirm: "PURGE-DURABLE-FRONTEND-TELEMETRY" });
  const preTestDepths = queueDepths();
  const dirtyBefore = Object.entries(preTestDepths).filter(([, depth]) => depth !== 0);
  if (dirtyBefore.length > 0) {
    findings.push(
      `queue state was not clean before test start after purge: ${JSON.stringify(dirtyBefore)}.`,
    );
  }
  const runId = `durable-delivery-volume-${crypto.randomUUID()}`;
  ...
  const dlqDepthsAfterDrain = Object.fromEntries(
    DLQ_QUEUES.map((queue) => [queue, queueDepth(queue)]),
  );
  for (const [queue, depth] of Object.entries(dlqDepthsAfterDrain)) {
    if (depth !== 0) findings.push(`DLQ ${queue} was not empty at test end (depth=${depth}).`);
  }

  const logs = dockerLogsForServices(["telemetry-ingest", "telemetry-delivery-worker"]);
  ...
  const evidencePath = writeEvidence("queue-storage-privacy.json", evidence);
  return { pass: findings.length === 0, findings, evidencePath, evidence };
}

async function verifyPreAckRedelivery() {
  const findings = [];
  const runId = `durable-delivery-preack-${crypto.randomUUID()}`;
  const marker = `${runId}-batch`;
  holdDelivery();
  runDockerCompose(["stop", "telemetry-delivery-worker"]);
  ...
  const harnessRun = ids
    ? runDockerCompose(
        [
          ...
          "--no-deps",
          "-v",
          `${repoRoot}:/workspace-test:ro`,
          "-e",
          `TEST_PREACK_EVENT_ID=${ids.eventId}`,
          "-e",
          `TEST_PREACK_BATCH_ID=${ids.batchId}`,
          "telemetry-delivery-worker",
          "node",
          "/workspace-test/scripts/lab/pre-ack-redelivery-worker.mjs",
        ],
        { capture: true, allowFailure: true },
      )
    : { status: 1, stdout: "", stderr: "missing admission ids" };
  const harnessLog = `${harnessRun.stdout ?? ""}${harnessRun.stderr ?? ""}`;
  const harnessExitedAtHook =
    harnessRun.status === 99 &&
    harnessLog.includes("pre_ack_redelivery_harness_exit_after_openobserve_success_before_ack");
  if (!harnessExitedAtHook) {
    findings.push(
  ...
  await waitForHealthy({ services: ["telemetry-delivery-worker"], timeoutMs: 90_000 });
  resumeDelivery();
  const drained = await waitForDrain(240_000);
  ...
  const evidence = {
    ...
    crashLogHash: sha256(harnessLog),
    ...
    accounting,
    preAckHarness: {
      mountedAtRuntime: true,
      bakedIntoProductionImage: false,
      expectedExitCode: 99,
      actualExitCode: harnessRun.status,
    },
  };
  const evidencePath = writeEvidence("pre-ack-redelivery.json", evidence);
  return { pass: findings.length === 0, findings, evidencePath, evidence };
}
...
async function verifyRealDurableOutageSoak() {
  const findings = [];
  const runId = `durable-delivery-soak-${crypto.randomUUID()}`;
  const accepted = [];
  const unexpected = [];
  resumeDelivery();
  await waitForHealthy({
  ...
  runDockerCompose(["stop", "openobserve"]);
  await waitForHealthy({
    services: ["reverse-proxy", "telemetry-ingest", "rabbitmq", "telemetry-delivery-worker"],
    timeoutMs: 90_000,
  });
  ...
  const evidencePath = writeEvidence("durable-outage-soak.json", evidence);
  return { pass: findings.length === 0, findings, evidencePath, evidence };
}
...
// invocation must fail fast with a clear message rather than silently queue behind the first.
...
if (isMainModule) {
  ...
  try {
    assertExactLabToolchain("test:durable delivery:durable-delivery");
    const result = await withExclusiveLock(
      RABBITMQ_EXCLUSIVE_LOCK_PATH,
      async () => {
        return command === "quick"
          ? await verifyDurableDelivery()
          : command === "queue-storage-privacy"
            ? await verifyQueueStoragePrivacy()
            : command === "pre-ack-redelivery"
              ? await verifyPreAckRedelivery()
              : command === "real-soak"
                ? await verifyRealDurableOutageSoak()
                : null;
      },
    ...
    if (!result) {
      throw new Error(
        `unknown durable-delivery command "${command}"; expected quick, queue-storage-privacy, pre-ack-redelivery, real-soak`,
      );
    }
    ...
    logError(
      `test:durable delivery:durable-delivery FAILED: ${error.message}` +
        (error.message.includes("could not acquire exclusive lock")
          ? " — run durable-delivery commands (quick, queue-storage-privacy, pre-ack-redelivery, real-soak) serially, never in parallel."
          : ""),
    );
    process.exit(1);
  }
