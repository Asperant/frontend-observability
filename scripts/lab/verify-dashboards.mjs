// `pnpm test:dashboards` — dashboard-governance acceptance gate. Combines:
...
//      exports) — dashboard-governance adds no new browser-facing API.
...
async function verifyFixtureMetricsAndEmptyData(auth, runId) {
  const findings = [];
  const fixtureService = `dashboard-governance-fixture-${runId}`;
  const env = "lab";
  const nowUs = NOW_US();
...
async function verifyInstallLifecycle(auth) {
  ...
  const companyTitle = `Company Dashboard Governance Probe`;
...
async function verifyLiveAuditDetectsRisk(auth) {
  ...
  const unsafeDashboard = {
    version: 3,
    dashboardId: "",
    title: "Dashboard Governance Disposable Unsafe Probe",
...
function escapeSqlLiteral(value) {
  if (typeof value !== "string" || !SQL_LITERAL_PATTERN.test(value)) {
    throw new Error(`unsafe dashboard-governance canary SQL literal: ${JSON.stringify(value)}`);
  }
...
async function driveCanaryBrowser(browserType, testRunId) {
  ...
  try {
    ...
    // established in scripts/lab/verify-streams.mjs — see that
...
export async function verifyDashboards() {
  const findings = [];
  const secretValues = readAdminSecretValues();
  const auth = readAdminAuthHeader();

  log("test:dashboards — running coverage-checked unit suite for scripts/lab/dashboards/...");
  ...
  log("test:dashboards — metric/query catalog validation...");
  findings.push(...verifyCatalogValidation());

  log("test:dashboards — known-fixture metric correctness + empty-data semantics...");
  ...
  log(
    "test:dashboards — starter install lifecycle (idempotency, company preservation, deletion respected, restore)...",
  );
  findings.push(...(await verifyInstallLifecycle(auth)));

  log("test:dashboards — export/backup secret-safety + import conflict guard...");
  findings.push(...(await verifyExportBackupImport(secretValues)));

  log("test:dashboards — live dashboard-audit risk detection...");
  findings.push(...(await verifyLiveAuditDetectsRisk(auth)));

  log("test:dashboards — management-plane isolation...");
  findings.push(...(await verifyManagementIsolation(auth)));

  log("test:dashboards — public API surface unchanged...");
  findings.push(...(await verifyPublicApiUnchanged()));

  log("test:dashboards — Chromium real query canary...");
  findings.push(...(await runBrowserQueryCanary("chromium", chromium, auth)));

  log("test:dashboards — Firefox real query canary...");
  findings.push(...(await runBrowserQueryCanary("firefox", firefox, auth)));

  assertNoSecretLeak(
    JSON.stringify(findings),
    secretValues,
    "a dashboard-governance finding string",
  );
...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:dashboards");
    const result = await verifyDashboards();
    if (result.pass) {
      log("\n✔ dashboard-governance dashboard/query governance verification PASSED.");
    } else {
      logError("\n✖ dashboard-governance dashboard/query governance verification FAILED:");
      for (const finding of result.findings) logError(`  - ${finding}`);
    }
    process.exit(result.pass ? 0 : 1);
  } catch (error) {
    logError(`test:dashboards FAILED: ${error.message}`);
    process.exit(1);
  }
}
