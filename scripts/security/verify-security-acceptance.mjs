...
} from "../../tests/security/adversarial/lab-adversarial.js";
...
function runPureLogicSuite() {
  const result = spawnSync(
    ...
    [
      ...
      "vitest",
      "run",
      "tests/security/adversarial/browser-package-adversarial.test.js",
      "tests/security/adversarial/governance-audit-adversarial.test.js",
    ],
    { encoding: "utf8" },
  );
...
export async function verifySecurityAcceptance() {
  let overallPass = true;
  const report = [];

  log("▶ security-acceptance: pure-logic adversarial suite (browser-package + governance-audit)");
  ...
  if (overallPass) {
    log("\n✔ security-acceptance PASSED — no open adversarial findings.");
  } else {
    logError("\n✖ security-acceptance FAILED.");
  }
...
if (isMainModule) {
  try {
    assertExactLabToolchain("test:security-acceptance");
    const { pass } = await verifySecurityAcceptance();
    process.exit(pass ? 0 : 1);
  } catch (error) {
    logError(`test:security-acceptance FAILED: ${error.message}`);
    process.exit(1);
  }
}
