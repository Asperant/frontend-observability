// `pnpm lab:resilience:soak [--duration=60m]` — mixed-workload soak run:
...
// `--duration=1h`); defaults to 60m per resilience's standard profile.
// Writes a secret-safe report to .runtime/resilience/ (gitignored, never
...
async function runBurstProbe() {
  const responses = await Promise.all(
    Array.from({ length: 60 }, (_, index) =>
      requestProxy(LOGS_PATH, { body: logPayload(`resilience-soak-burst-${index}`) }),
    ),
  );
...
async function runKillSwitchCycle() {
  ...
  try {
    const blocked = await requestProxy(LOGS_PATH, {
      body: logPayload("resilience-soak-kill-switch-blocked"),
    });
...
export async function runResilienceSoak({ durationMs }) {
  ...
  log(
    `lab:resilience:soak — running for ${durationMs}ms (ends ${new Date(endAtMs).toISOString()})`,
  );
  ...
  log(
    `lab:resilience:soak — report written to ${outPath} (not committed, .runtime/ is gitignored)`,
  );
  return report;
}
...
if (isMainModule) {
  try {
    assertExactLabToolchain("lab:resilience:soak");
    const { durationMs } = parseArgs(process.argv.slice(2));
    const report = await runResilienceSoak({ durationMs });
    log(
      `lab:resilience:soak complete. anyLeakFlagged=${report.anyLeakFlagged} restartCycle.healthRecovered=${report.restartCycle?.healthRecovered} unexpectedDrop=${report.traffic.unexpectedDrop} finalMarkerVisible=${report.finalMarker.visibilityMs !== null}`,
    );
    ...
    logError(`lab:resilience:soak FAILED: ${error.message}`);
    process.exit(1);
  }
}
