// `pnpm test:stage19:resilience` — Stage 19 acceptance gate: performance
// budgets, restart-recovery timing/drift, notification readiness, and
// failure isolation. Short and deterministic (per Stage 19's own scope) —
...
function rumPayload(overrides = {}) {
  return JSON.stringify({
    ...
    view: { id: crypto.randomUUID(), url: `${PROXY_ORIGIN}/stage19` },
...
function runLibCoverageSuite() {
  const result = spawnSync(
    ...
    [
      ...
      "--project",
      "lab",
      "tests/performance/stage19/unit",
      "--coverage",
...
async function measureBrowserBudgetsForEngine(browserType) {
  ...
  // scripts/lab/verify-stage8-openobserve.mjs's identical init+consent
  ...
  // Seeds event kinds the Stage 16 query catalog's manifests need
...
export async function measureProxyLatency() {
  ...
  for (let i = 0; i < 40; i += 1) {
    const response = await requestProxy(LOGS_PATH, {
      body: JSON.stringify({
        date: Date.now(),
        message: "stage19-proxy-latency-probe",
        status: "info",
        service: DEMO_IDENTITY.service,
        env: DEMO_IDENTITY.environment,
        version: DEMO_IDENTITY.version,
...
export async function measureIngestionVisibility() {
  ...
  const canaryId = `stage19-ingest-canary-${crypto.randomUUID()}`;
  const sentAtMs = Date.now();
  const response = await requestProxy(RUM_PATH, {
    body: rumPayload({ view: { id: canaryId, url: `${PROXY_ORIGIN}/stage19-canary` } }),
  });
...
export async function measureQueryCatalogPerformance() {
  ...
  for (const manifest of manifests) {
    ...
    for (const drilldown of manifest.drilldownVariables ?? []) {
      variables[drilldown] = "stage19-nonexistent-session";
    }
...
const SERVICES_LIST = ["reverse-proxy", "demo-frontend", "mock-api", "alert-sink", "openobserve"];

/**
 * Creates one company-owned (non-starter, no Stage 15/16/17 marker) alert
...
async function createDisposableCompanyAlert() {
  ...
  const body = {
    ...
    name: `stage19-disposable-company-alert-${Date.now()}`,
    description: "Stage 19 resilience-gate disposable company-owned alert (no starter marker)",
  };
  const response = await createAlert(auth, body);
  if (response.status !== 200) {
    throw new Error(
      `failed to create the disposable company alert for Stage 19 (${response.status})`,
    );
  }
...
async function runRestartRecoveryMatrix() {
  const scenarios = [
    { target: "demo-frontend", options: {} },
    { target: "mock-api", options: {} },
...
export async function measureContainerResourceRecovery() {
  ...
  // with soak-grade sample counts lives in run-stage19-soak.mjs.
...
export async function verifyStage19Resilience() {
  assertExactLabToolchain("test:stage19:resilience");
  let overallPass = true;
  const findings = [];
  const measurements = {};

  log("▶ stage19: lib unit coverage suite (parser/collector/budget/recovery/readiness)");
  ...
  log("▶ stage19: browser budgets (Chromium + Firefox)");
  ...
  log("▶ stage19: proxy latency / error distribution");
  ...
  log("▶ stage19: controlled ingestion visibility canary");
  ...
  log("▶ stage19: network partition (docker network disconnect/connect on mock-api)");
  const partition = await runNetworkPartitionScenario();
  if (!partition.pass) {
    overallPass = false;
    findings.push(
      `network partition scenario failed: ${partition.reason ?? "mock-api did not recover"}`,
    );
  } else {
    log(`  PASS (recoveryTimeMs=${partition.recoveryTimeMs})`);
  }

  log("▶ stage19: kill switch under load");
  ...
  log("▶ stage19: container resource recovery");
  ...
  if (overallPass) {
    log("\n✔ stage19:resilience PASSED.");
  } else {
    logError("\n✖ stage19:resilience FAILED:");
