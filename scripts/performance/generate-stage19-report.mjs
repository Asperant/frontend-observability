// `node scripts/performance/generate-stage19-report.mjs <path-to-benchmark-or-soak-report.json>`
// Renders one raw .runtime/stage19/*.json report (benchmark or soak) into a
// secret-safe markdown summary suitable for pasting into
// docs/stage19-capacity-envelope.md. Never touches git; a human decides
...
export function renderStage19ReportMarkdown(report) {
  assertSecretSafe(report);
  const lines = [];
  lines.push(`## Stage 19 ${report.kind} report`);
  lines.push("");
...
if (isMainModule) {
  try {
    ...
    if (!inputPath) throw new Error("usage: generate-stage19-report.mjs <path-to-report.json>");
    const report = JSON.parse(readFileSync(inputPath, "utf8"));
    log(renderStage19ReportMarkdown(report));
  } catch (error) {
    logError(`generate-stage19-report FAILED: ${error.message}`);
    process.exit(1);
  }
}
