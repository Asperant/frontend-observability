// `pnpm lab:resilience:benchmark` — reference-lab measurement run. Reuses the
// exact same real measurement functions as test:resilience (browser
...
// secret-safe report to .runtime/resilience/ (gitignored, never auto-committed).
...
export async function runResilienceBenchmark({ profile = "normal" } = {}) {
  const environment = collectEnvironmentFingerprint({ workloadProfile: profile });

  log("lab:resilience:benchmark — browser budgets (Chromium + Firefox)...");
  const browser = await measureBrowserBudgets();

  log("lab:resilience:benchmark — proxy latency...");
  const proxy = await measureProxyLatency();

  log("lab:resilience:benchmark — ingestion visibility canary...");
  const ingestion = await measureIngestionVisibility();

  log("lab:resilience:benchmark — 26-query catalog...");
  const queryCatalog = await measureQueryCatalogPerformance();

  log("lab:resilience:benchmark — notification readiness...");
  const notification = await runNotificationReadinessProbe();

  log("lab:resilience:benchmark — container resource snapshot...");
  ...
  log(
    `lab:resilience:benchmark — report written to ${outPath} (not committed, .runtime/ is gitignored)`,
  );
  return report;
}
...
if (isMainModule) {
  try {
    assertExactLabToolchain("lab:resilience:benchmark");
    await runResilienceBenchmark();
    log("lab:resilience:benchmark complete.");
  } catch (error) {
    logError(`lab:resilience:benchmark FAILED: ${error.message}`);
    process.exit(1);
  }
}
